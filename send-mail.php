<?php
session_start();
ini_set("display_errors",1);
use PHPMailer\PHPMailer\PHPMailer;
use PHPMailer\PHPMailer\Exception;
use PHPMailer\PHPMailer\SMTP;

require 'PHPMailer/src/Exception.php';
require 'PHPMailer/src/PHPMailer.php';
require 'PHPMailer/src/SMTP.php';

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Retrieve form data
    require_once __DIR__ . '/lib/AntiSpam.php';
    $antiSpam = new AntiSpam();
    $emailFieldName = $antiSpam->getEmailFieldName();
    
    // Retrieve form data
    $fname = $_POST['fname'];
    $lname = $_POST['lname'];
    $email = $_POST[$emailFieldName];
    $phone = $_POST['phone'];
    $address = $_POST['your-address'];
    $service = $_POST['your-service'];
    $message = $_POST['message'];
    
 
    // Perform form validation if required

    // You can add your validation logic here
    // security validations
    require_once __DIR__ . '/lib/FormService.php';
    $formService = new FormService();

    $formService->validateCSRFAttack();
    $formService->validateBruteForceAttack($_POST["form_token"]);
    
    $honeyPotFieldName = $antiSpam->getHoneyPotFieldName();

    $honeyPot = $_POST[$honeyPotFieldName];
    if (isset($_POST["email"])) {
        $formService->validateBotSpam($honeyPot, $_POST["email"]);
    }
    
    $isStopwordsFree = $formService->validateStopWords($message);
    
    if($isStopwordsFree) {
        // Call the sendEmail function to send the email
        $isSent = sendEmail($fname,$lname, $email, $phone,$address,$service, $message);
    }

    if($isSent) {
        // Respond with a success message
        $success = 'Thank you for contacting us.  We will respond at our earliest opportunity.';
        $output = $formService->createJsonInstance($success, 'success');
    } else {
        $output = $formService->createJsonInstance("Failed to send the email. Please try again later.", 'error');
    }
    $formService->endAction($output);
}



function sendEmail($fname,$lname, $email, $phone,$address,$service, $message)
{
    $isSent = false;
    // Replace this with your static 'From' address


    // Create an instance; passing `true` enables exceptions
    $mail = new PHPMailer(true);

    try {
        // Server settings
        $mail->SMTPDebug = 0; // Enable verbose debug output
        $mail->isSMTP();
        $mail->Host = 'smtp.gmail.com';
        $mail->SMTPAuth = true;
        $mail->Username = 'wtsslead@gmail.com';
        $mail->Password = 'xixy vnzh vbrh wbmo'; // Replace with your generated App Password
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
        $mail->Port = 587;


        $timestamp = time();


       $subject = 'Enquiry through the website - ESV Residential';

        // Sender
        $mail->setFrom($email,$fname);
        
        // Reply-To
        $mail->addReplyTo($email, $fname);

        // Recipients
        $mail->addAddress('info@esvent.ca','East Side Ventilation');

        // CC recipients
        $mail->addCC('EastSideVent@gmail.com', 'East Side Ventilation');

      // BCC recipients
        $mail->addBCC('simonwtss@gmail.com', 'Simon');
        $mail->addBCC('website-leads@outlook.com', 'Simon');
    
  
        


        // Content
        $mail->isHTML(true); // Set email format to HTML
        $mail->Subject = $subject . ' - ' . $timestamp;

        // Compose the email message using an HTML table
        $mailBody = '<html>';
        $mailBody .= '<body>';
        $mailBody .= '<table style="border: 1px solid #afa4a4;padding: 10px;width: 100%; max-width: 600px;">';
           $mailBody .= '<tr><td>First Name:</td><td>' . $fname . '</td></tr>';
        $mailBody .= '<tr><td>Last Name:</td><td>' . $lname . '</td></tr>';
        $mailBody .= '<tr><td>Email:</td><td>' . $email . '</td></tr>';
        $mailBody .= '<tr><td>Phone:</td><td>' . $phone . '</td></tr>';
        $mailBody .= '<tr><td>Address:</td><td>' . $address . '</td></tr>';
        $mailBody .= '<tr><td>Service:</td><td>' . $service . '</td></tr>';
        $mailBody .= '<tr><td>Message:</td><td>' . $message . '</td></tr>';
        $mailBody .= '</table>';
        $mailBody .= '</body>';
        $mailBody .= '</html>';

        $mail->Body = $mailBody;
        $mail->AltBody = 'This is the body in plain text for non-HTML mail clients';

        // Send email
        $isSent = $mail->send();
        //echo 'Message has been sent. ';
    } catch (Exception $e) {
        echo "Message could not be sent. Mailer Error: {$mail->ErrorInfo}";
    }
    return $isSent;
}
?>